...
def get_doc_processor() -> DocumentProcessor:
    """按需创建文档处理器，避免模块导入时直接触发外部依赖初始化。"""
    global doc_processor
    if doc_processor is None:
        try:
            doc_processor = DocumentProcessor(
                persist_directory=os.getenv("PERSIST_DIR", "./vector_store")
            )
        except Exception as exc:
            logger.error(f"初始化文档处理器失败: {exc}", exc_info=True)
            raise HTTPException(
                status_code=503,
                detail=f"文档处理器初始化失败: {exc}",
            )
...
async def add_urls(request: UrlRequest):
    ...
    try:
        ...
        result = await processor.add_urls(request.urls)
